...
func loadAllSnapshots(ctx context.Context, repo restic.Repository, excludeIDs restic.IDSet) (snapshots data.Snapshots, err error) {
	err = data.ForAllSnapshots(ctx, repo, excludeIDs, func(id restic.ID, sn *data.Snapshot, err error) error {
		if err != nil {
			return err
		}
...
func TestCreateSnapshot(t *testing.T) {
	...
	for i := 0; i < testCreateSnapshots; i++ {
		data.TestCreateSnapshot(t, repo, testSnapshotTime.Add(time.Duration(i)*time.Second), testDepth, 0)
	}
