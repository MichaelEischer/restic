...
func (l *Lock) checkForOtherLocks(ctx context.Context) error {
	...
	for i := 0; i < 3; i++ {
		err = ForAllLocks(ctx, l.repo, l.lockID, func(id restic.ID, lock *Lock, err error) error {
			if err != nil {
...
func (l *Lock) Unlock() error {
	...
	return l.repo.Backend().Remove(context.TODO(), restic.Handle{Type: restic.LockFile, Name: l.lockID.String()})
}
...
func (l *Lock) Refresh(ctx context.Context) error {
	...
	return l.repo.Backend().Remove(context.TODO(), restic.Handle{Type: restic.LockFile, Name: oldLockID.String()})
}
...
func RemoveStaleLocks(ctx context.Context, repo restic.Repository) (uint, error) {
	var processed uint
	err := ForAllLocks(ctx, repo, nil, func(id restic.ID, lock *Lock, err error) error {
		...
		if lock.Stale() {
			err = repo.Backend().Remove(ctx, restic.Handle{Type: restic.LockFile, Name: id.String()})
			if err == nil {
				processed++
			}
			return err
...
func ForAllLocks(ctx context.Context, repo restic.Repository, excludeID *restic.ID, fn func(restic.ID, *Lock, error) error) error {
	var m sync.Mutex
