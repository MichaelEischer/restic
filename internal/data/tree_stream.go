...
func filterTrees(ctx context.Context, repo Loader, trees restic.IDs, loaderChan chan<- trackedID, hugeTreeLoaderChan chan<- trackedID,
	...
	for {
		if loadCh == nil && len(backlog) > 0 {
			...
			treeSize, found := repo.LookupBlobSize(nextTreeID.ID, restic.TreeBlob)
...
func StreamTrees(ctx context.Context, wg *errgroup.Group, repo Loader, trees restic.IDs, skip func(tree restic.ID) bool, p *progress.Counter) <-chan TreeItem {
