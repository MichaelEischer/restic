...
func TestLock(t *testing.T) {
	...
	lock, err := data.NewLock(context.TODO(), repo)
	rtest.OK(t, err)
...
func TestMultipleLock(t *testing.T) {
	...
	lock1, err := data.NewLock(context.TODO(), repo)
	rtest.OK(t, err)

	lock2, err := data.NewLock(context.TODO(), repo)
	rtest.OK(t, err)
...
func TestMultipleLockFailure(t *testing.T) {
	...
	_, err = data.NewLock(context.TODO(), repo)
...
func TestLockExclusive(t *testing.T) {
	...
	elock, err := data.NewExclusiveLock(context.TODO(), repo)
	rtest.OK(t, err)
...
func TestLockOnExclusiveLockedRepo(t *testing.T) {
	...
	rtest.Assert(t, data.IsAlreadyLocked(err),
...
func TestExclusiveLockOnLockedRepo(t *testing.T) {
	...
	elock, err := data.NewLock(context.TODO(), repo)
	rtest.OK(t, err)

	lock, err := data.NewExclusiveLock(context.TODO(), repo)
	rtest.Assert(t, err != nil,
		"create normal lock with exclusively locked repo didn't return an error")
	rtest.Assert(t, data.IsAlreadyLocked(err),
		"create normal lock with exclusively locked repo didn't return the correct error")
...
func TestLockWithStaleLock(t *testing.T) {
	...
	processed, err := data.RemoveStaleLocks(context.TODO(), repo)
	rtest.OK(t, err)
...
func TestRemoveAllLocks(t *testing.T) {
	...
	processed, err := data.RemoveAllLocks(context.TODO(), repo)
	rtest.OK(t, err)
...
func TestLockRefresh(t *testing.T) {
	...
	lock2, err := data.LoadLock(context.TODO(), repo, *lockID2)
	rtest.OK(t, err)
