...
func TestFindLatestSnapshot(t *testing.T) {
	...
	sn, err := restic.FindFilteredSnapshot(context.TODO(), repo, []string{"foo"}, []restic.TagList{}, []string{}, nil, "latest")
	if err != nil {
...
func TestFindLatestSnapshotWithMaxTimestamp(t *testing.T) {
	...
	sn, err := restic.FindFilteredSnapshot(context.TODO(), repo, []string{"foo"}, []restic.TagList{}, []string{}, &maxTimestamp, "latest")
	if err != nil {
		t.Fatalf("FindLatestSnapshot returned error: %v", err)
	}
