...
func loadAndCompare(t testing.TB, be backend.Backend, h backend.Handle, data []byte) {
	buf, err := restic.LoadAll(context.TODO(), nil, be, h)
	if err != nil {
		t.Fatal(err)
	}
...
func (be loadErrorBackend) Load(ctx context.Context, h backend.Handle, length int, offset int64, fn func(rd io.Reader) error) error {
...
func TestBackendRemoveBroken(t *testing.T) {
	...
	buf, err := restic.LoadAll(context.TODO(), nil, c.Wrap(be), h)
	test.OK(t, err)
